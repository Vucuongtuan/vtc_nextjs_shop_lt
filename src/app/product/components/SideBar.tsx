...
export const SideBar = ({ dataBrand, params }: any) => {
  ...
  useEffect(() => {
    ...
    if (brandNameParam && idBrandParam) {
      const index = dataBrand.findIndex(
        (brand: any) =>
  ...
  return (
    <div className="p-0 pr-[30px] basis-[25%] max-w-[25%] bg-white mr-2 rounded-md">
      <div className="align-baseline ">
        <div className="w-full h-auto p-2">
          <span className="mb-4">Hãng sản xuất {checkBrand}</span>
          <div className="flex items-center flex-wrap flex-row ">
            {dataBrand?.map((brand: any, index: number) => (
              <div
